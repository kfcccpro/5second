(() => {
  ...
  function renderPlan(plan){if(!$('todayDay'))return;$('todayDay').textContent=`${fmtDate(plan.date)} · ${plan.dayId}`;$('todayTime').textContent=`약 ${plan.targetMinutes}분`;$('grammarTime').textContent=`${plan.grammarMinutes}분`;$('readingTime').textContent=`${plan.readingMinutes}분`;$('todayCount').textContent=`${window.JK_DAILY.planQuestionIds(plan).length}개 활동`;const b=$('todayStartBtn');if(b){b.textContent=plan.status==='completed'?'오늘 학습 완료':plan.status==='active'?'중단한 곳부터 이어하기':'오늘 학습 시작';b.disabled=plan.status==='completed'}const adaptive=$('adaptivePlanNote');if(adaptive){adaptive.textContent=plan.adaptiveRecommendationId?'관리자가 확인한 학습 조정이 오늘 계획에 반영되었습니다.':'오늘 계획이 준비되었습니다.';adaptive.classList.toggle('hidden',!plan.adaptiveRecommendationId);}}
  ...
  window.addEventListener('jk-sync-status',e=>{const el=$('syncState');if(!el)return;el.textContent=window.JK_PHASE31?.compactStatus(e.detail.status)||'이 기기에 저장 중'});
  window.JK_PHASE20={ensurePlan,refreshHome,finalizeDay,renderReport};
  window.JK_PHASE19=window.JK_PHASE20;window.JK_PHASE18=window.JK_PHASE20;
