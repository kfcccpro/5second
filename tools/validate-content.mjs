...
const FORBIDDEN_UI_CHOICES=new Set([
...
const forbidden=(choices=[])=>choices.map(norm).filter(x=>FORBIDDEN_UI_CHOICES.has(x));
...
for(const q of D.questions){
  ...
  const badMain=forbidden(q.choices);if(badMain.length)errors.push(`${q.id}: UI control label leaked into final choices: ${badMain.join(', ')}`);
  ...
  for(const s of q.decisionSteps||[]){
    ...
    const badStep=forbidden(s.choices);if(badStep.length)errors.push(`${q.id}/${s.id||'?'}: UI control label leaked into process choices: ${badStep.join(', ')}`);
    if(new Set((s.choices||[]).map(norm)).size!==(s.choices||[]).length)errors.push(`${q.id}/${s.id||'?'}: duplicate process choices`);
  }
  ...
  if(c){
    ...
    const badCheck=forbidden(c.choices);if(badCheck.length)errors.push(`${q.id}: UI control label leaked into checkQuestion choices: ${badCheck.join(', ')}`);
